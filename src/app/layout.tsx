...
export default function RootLayout({children}: Readonly<{ children: React.ReactNode; }>) {
    useEffect(() => {
        const signOutOnClose = async (): Promise<void> => {
            const token = sessionStorage.getItem("token");

            if (!token) return;

            const res = await fetch(process.env.api + "/auth/signout", {
                method: "POST",
                headers: {
                    token: token,
                },
    ...
    return (
        ...
        <head>
            ...
            <meta name="description" content="Modern Database Management" />
        </head>
                ...
                <div className="max-w-7xl mx-auto px-6 h-14 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-1 text-sm font-medium text-gray-500 px-3">
                            ...
                            <span className="text-gray-300">•</span>
                            <Link href="/auth/signin" className="hover:text-black transition-colors">Admin</Link>
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <div
                            className="w-8 h-8 rounded-full bg-gradient-to-tr from-blue-500 to-purple-500 shadow-inner"></div>
                    </div>
                </div>
            </nav>
